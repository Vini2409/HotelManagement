...
<%
    ...
    int status= authendication.memberLogin(obj);
    obj.setMemberid(obj.getMemberid());
    if(status>0) {
        out.println("Login credential success");
        out.println("</br></br>");
        session.setAttribute("validid",obj.getMemberid());
//        out.println(template.append("<form action=memberHomePage.jsp>").append("<Button name='gname' value=").append(obj.getGuestid()).append(">GO TO HOME PAGE</input>"));
        out.println("<a href='memberHomePage.jsp'>GO TO Home PAGE</a>");
    }
    else {
